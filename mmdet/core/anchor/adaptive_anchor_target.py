...
def aa_center_target(gt_bboxes_list,
    ...
    for img_id in range(img_per_gpu):
        ...
        for gt_id in range(gt_bboxes.size(0)):
            ...
            all_center_targets[lvl][img_id, 0, ctr_y1:ctr_y2 + 1, ctr_x1:ctr_x2 +
                                                                         1] = 1
            all_center_weights[lvl][img_id, 0, ignore_y1:ignore_y2 +
                                                         1, ignore_x1:ignore_x2 + 1] = 0
            all_center_weights[lvl][img_id, 0, ctr_y1:ctr_y2 + 1, ctr_x1:ctr_x2 +
                                                                         1] = 1
    ...
    center_avg_factor = sum(
        [t.size(0) * t.size(-1) * t.size(-2) for t in all_center_targets]) / 200
    return all_center_targets, all_center_weights, center_avg_factor


def aa_shape_target(approx_list,
                    inside_flag_list,
                    square_list,
                    gt_bboxes_list,
                    img_metas,
...
def aa_shape_target_single(flat_approxs,
                           inside_flags,
                           flat_squares,
                           gt_bboxes,
                           gt_bboxes_ignore,
