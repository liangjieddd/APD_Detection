...
from mmdet.core import (AnchorGenerator, anchor_inside_flags, anchor_target,
                        delta2bbox, force_fp32, aa_center_target, aa_shape_target,
...
class FeatureMerged(nn.Module):
    ...
    def __init__(self,
                 ...
                 kernel_size=1,
                 deformable_groups=4):
        super(FeatureMerged, self).__init__()
...
class AdaptiveAnchorHead(AnchorHead):
    """
    Args:
        ...
        center_filter_thr (float): Threshold to filter out unconcerned regions.
        loss_center (dict): Config of centeration loss.
    ...
    def __init__(
            ...
            loss_center=dict(
                ...
                alpha=0.25,
                loss_weight=1.0),
            loss_shape=dict(type='SmoothL1Loss', beta=0.2, loss_weight=1.0),
            loss_cls=dict(
        ...
        self.center_filter_thr = center_filter_thr
        ...
        # one anchor per centeration
        self.num_anchors = 1
        self.use_sigmoid_cls = loss_cls.get('use_sigmoid', False)
        self.cls_focal_loss = loss_cls['type'] in ['FocalLoss']
        self.center_focal_loss = loss_center['type'] in ['FocalLoss']
    ...
    def _init_layers(self):
        self.relu = nn.ReLU(inplace=True)
        self.conv_center = nn.Conv2d(self.in_channels, 1, 1)
        self.conv_shape = nn.Conv2d(self.in_channels, self.num_anchors * 2, 1)
        self.feature_merge = FeatureMerged(
            self.in_channels,
            self.feat_channels,
            kernel_size=3,
    ...
    def init_weights(self):
        ...
        normal_init(self.conv_center, std=0.01, bias=bias_cls)
        normal_init(self.conv_shape, std=0.01)

        self.feature_merge.init_weights()

    def forward_single(self, x):
        center_pred = self.conv_center(x)
        shape_pred = self.conv_shape(center_pred)
        x = self.feature_merge(x, shape_pred)
        # masked conv is only used during inference for speed-up
        if not self.training:
            mask = center_pred.sigmoid()[0] >= self.center_filter_thr
        else:
            mask = None
        cls_score = self.conv_cls(x, mask)
        bbox_pred = self.conv_reg(x, mask)
        return cls_score, bbox_pred, shape_pred, center_pred
    ...
    def get_anchors(self,
        ...
        Args:
            ...
            center_preds (list[tensor]): Multi-level centeration predictions.
            img_metas (list[dict]): Image meta info.
            use_center_filter (bool): Use center filter or not.
            device (torch.device | str): device for returned tensors
        ...
        # for each image, we compute multi level adaptive anchors
        adaptive_anchors_list = []
        center_mask_list = []
        for img_id, img_meta in enumerate(img_metas):
            multi_level_adaptive_anchors = []
            multi_level_center_mask = []
            for i in range(num_levels):
                squares = squares_list[img_id][i]
                shape_pred = shape_preds[i][img_id]
                center_pred = center_preds[i][img_id]
                adaptive_anchors, center_mask = self.get_adaptive_anchors_single(
                    squares,
                    shape_pred,
                    center_pred,
                    use_center_filter=use_center_filter)
                multi_level_adaptive_anchors.append(adaptive_anchors)
                multi_level_center_mask.append(center_mask)
            adaptive_anchors_list.append(multi_level_adaptive_anchors)
            center_mask_list.append(multi_level_center_mask)
        return squares_list, adaptive_anchors_list, center_mask_list
    ...
    def loss_center_single(self, center_pred, center_target, center_weight, center_avg_factor,
                           cfg):
        loss_center = self.loss_center(
            center_pred.reshape(-1, 1),
            center_target.reshape(-1, 1).long(),
            center_weight.reshape(-1, 1),
            avg_factor=center_avg_factor)
        return loss_center

    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'shape_preds', 'center_preds'))
    def loss(self,
             cls_scores,
             bbox_preds,
             shape_preds,
        ...
        # get center targets
        center_targets, center_weights, center_avg_factor = aa_center_target(
            gt_bboxes,
            featmap_sizes,
        ...
        # get squares and adaptive anchors
        squares_list, adaptive_anchors_list, _ = self.get_anchors(
            featmap_sizes, shape_preds, center_preds, img_metas, device=device)

        # get shape targets
        sampling = False if not hasattr(cfg, 'aa_sampler') else True
        shape_targets = aa_shape_target(
            approxs_list,
            inside_flag_list,
            squares_list,
            gt_bboxes,
        ...
        # get anchor centeration loss
        losses_center = []
        for i in range(len(center_preds)):
            loss_center = self.loss_center_single(
                center_preds[i],
                center_targets[i],
                center_weights[i],
                center_avg_factor=center_avg_factor,
                cfg=cfg)
            losses_center.append(loss_center)
    ...
    def get_bboxes(self,
        ...
        _, adaptive_anchors, center_masks = self.get_anchors(
            featmap_sizes,
            shape_preds,
            center_preds,
            img_metas,
            use_center_filter=not self.training,
            device=device)
        result_list = []
        for img_id in range(len(img_metas)):
            ...
            adaptive_anchor_list = [
                adaptive_anchors[img_id][i].detach() for i in range(num_levels)
            ]
            center_mask_list = [
                center_masks[img_id][i].detach() for i in range(num_levels)
            ]
            img_shape = img_metas[img_id]['img_shape']
            scale_factor = img_metas[img_id]['scale_factor']
            proposals = self.get_bboxes_single(cls_score_list, bbox_pred_list,
                                               adaptive_anchor_list,
                                               center_mask_list, img_shape,
